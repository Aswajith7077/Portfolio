...
const Contact = () => {
  ...
  return <div>
      ...
      duration:0.5,
      delay:0.25
    }}}  className="flex flex-col lg:flex-row w-full gap-10 py-2 items-center justify-center">

            <input type="text" placeholder="First Name" className="w-full lg:w-[30%] bg-transparent border-b-2 outline-none px-5 py-4" onChange={e => setFirstName(e.target.value)} />
            <input type="text" placeholder="Last Name" className="w-full lg:w-[30%] bg-transparent border-b-2 outline-none px-5 py-4" onChange={e => setLastName(e.target.value)} />
          </motion.div>
          <motion.div initial={{x:-100,opacity:0}} whileInView={{x:0,opacity:1,transition:{
      duration:0.5,
      delay:0.5
    }}}  className="flex flex-col lg:flex-row justify-center items-center gap-10">
            <input type="email" placeholder="Email" className="w-full lg:w-[30%] bg-transparent border-b-2 outline-none px-5 py-4" onChange={e => setEmail(e.target.value)} />
            <input type="phone" placeholder="Number" className="w-full lg:w-[30%] bg-transparent border-b-2 outline-none px-5 py-4" onChange={e => setPhone(e.target.value)} />
          </motion.div>
          <motion.textarea initial={{x:-100,opacity:0}} whileInView={{x:0,opacity:1,transition:{
      duration:0.5,
      delay:0.75
    }}}  rows={5} placeholder="Your Message" onChange={e => setMessage(e.target.value)} className="w-full lg:w-[65%] lg:mx-[18%] bg-[#0f172a] px-5 py-4 my-2 rounded-[15px] " />
          <motion.button initial={{x:-100,opacity:0}} whileInView={{x:0,opacity:1,transition:{
      duration:0.5,
      delay:0.75
    }}} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex py-4 px-7 w-full lg:w-[30%] lg:mx-[18%] flex-row items-center gap-3 font-[lato] border-2 font-semibold rounded-3xl text-white text-lg justify-center">
            Send Message
          </motion.button>
        </form>
        <Address />
